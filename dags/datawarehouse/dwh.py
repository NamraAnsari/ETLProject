...
def staging_table():
    ...
    try:
        ...
        for row in YT_data:
            if len(table_ids) == 0:
                insert_rows(conn, cur, schema, row)

            else:
                if row['video_id'] is table_ids:
                    update_rows(conn, cur, schema, row)
    ...
    try:
        ...
        for row in rows:
            ...
            if len(table_ids) == 0:
                ...
                insert_rows(conn, cur, schema, transformed_row)

            else:
                transformed_row = transform_data(row)

                if transformed_row["video_id"] in table_ids:
                    update_rows(conn, cur, schema, transformed_row)
                else:
                    insert_rows(conn, cur, schema, transformed_row)
...
def trunc_table():
    conn, cur = get_conn_cursor()
    del_staging = f"""TRUNCATE TABLE staging.{table};"""
    del_core = f"""TRUNCATE TABLE core.{table};"""
    cur.execute(del_staging)
    cur.execute(del_core)
    conn.commit()
    close_conn_cursor(conn,cur)